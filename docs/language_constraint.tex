...
where annotators were assigned similar writing tasks with different constraints: (1) writing an entire story, (2) adding a story ending for a given story context, and (3) adding an incoherent ending to a story.\resolved{\clinic{Many felt these 2 contraints looks like 3 constraints. clarify}}
...
In addition, our style-based classifier obtains state-of-the-art results on the story cloze challange, substantially higher than  deep learning models, without even looking at the story context. 
...
its annotation process introduced three variants of the same writing task---the {\it original}, {\it right} and {\it wrong} endings. 
...
\ms{The proper noun thing is prolly related to the task design, which stated that people had to use at least one of the characters; research shows people usually use more other references when lying, but it's not clear whether there's a pronoun/proper noun distinction.}\roy{Good point! This is tricky. On the one hand, it hurts our story by saying that the difference is not really the general task by itself, but also the constraints put by  Nasrin et al. It also forces us to bash them some more. Or we can just ignore it...} 
...
Furthermore, {\it original} and {\it right} endings seem to prefer enthusiastic language (e.g., ``!''), while {\it wrong} endings tend to use more negative language (``hates''), as is deceptive text \cite{Newman:2003}.
