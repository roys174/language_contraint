...
slightly longer than {\it wrong} ones (8.47 words). This is inline with findings by \citet{yancheva2013automatic,qin2004exploratory}, which found that sentence length was indicative of whether a text was deceptive. Furthermore, \citet{buller1996interpersonal} suggests lying increases one's cognitive load, which in turn could affect sentence length.
...
\ms{The proper noun thing is prolly related to the task design, which stated that people had to use at least one of the characters; research shows people usually use more other references when lying, but it's not clear whether there's a pronoun/proper noun distinction.}
