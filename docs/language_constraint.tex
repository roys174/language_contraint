...
where annotators were assigned similar writing tasks with different constraints: (1) writing an entire story on their own vs.~adding only a story ending for a given story context and (2) writing a story ending that makes the overall story coherent or incoherent. 
...
They also provide important lessons for designing new NLP
tasks. \nascomment{drop this sentence if we only have a paragraph
  about this at the end, as we currently do}

\end{abstract}

\nascomment{resolve:  story cloze or cloze story?  we go back and
  forth.  use the standard one and capitalize it consistently.}
...
the nature of the writing task she was asked to perform, since
different tasks likely engage different cognitive processes.
...
Our analysis reveals that \nascomment{feels like a leap here, maybe
  ...
  from weights alone } sentiment plays a significant role, such that authors writing the wrong endings often engage negative sentiment. 
...
Sections \ref{ssec:Ablation} and \ref{ssec:Discussion} present an
ablation study and a discussion, followed by related work and conclusions.
...
The resulting stories, both {\it right} and {\it wrong}, were then
individually rated for coherence and meaningfulness \nascomment{by
...
paper was first published, the published benchmark on this task is
...
\yc{SQuAD was officially published in Nov 2016. by the time the
  official EMNLP talk was presented, the leaderboard performance
  (still unpublished) was  30\% beyond the best results in the
  paper. Given that, the statement of this paragraph seems a bit of
  overkill...?} \nascomment{it sounds like Yejin is saying that on
  squad, there was an even faster acceleration?  or that we should
  change ``similar period of time'' to ``similar or shorter period of time''?}
...
\yc{As far as I understand, this was intentional --- they didn't want
...
In this work, we show that these differences are significant and
...
First, {\it original} endings are on average much longer (11 words per
...
We train a logistic regressionclassifier to distinguish between different endings. 
...
As the cloze story task doesn't have a training corpus for the {\it
...
By ignoring the coupling between {\it right} and {\it wrong} pairs, we
...
We use the endings of the ROC stories as our {\it original} training
samples and {\it right} endings from the story cloze task  development
and test sets \nascomment{if you're using the test examples as
  ...
  you'd follow the same pattern as above ...} as {\it new} training samples.
...
\emph{new} instances, for each of the training, development, and test sets.
...
regularization, performing grid search on the development test to
...
published state-of-the-art result on the task \nascomment{cite that here}.
...
task affects her writing style in easily detected ways.
...
Some of these works showed that these changes \nascomment{what are
  ``these changes''?} are also accompanied by changes in writing style
\cite{Campbell:2003}. The current study suggests that even subtle
changes to a writing prompt can affect these processes.
...
Luckily, the substantial improvement we get when combining our model
with the RNN language model (\secref{Experiments}) suggests that
despite the high results obtained by the simple model, the
more recent RNN approach captures a different aspect of the
...
The cloze story task, which is the focus of this paper, is part of a wide set of machine reading works published in the last few years.
...
problems, such as inherent ambiguity caused by applying automatic
tools \nascomment{this is not clear to me, what does it mean?} when generating the datasets \cite{Chen:2016}. 
