...
It is influenced by personal factors such as age \cite{Schler:2006} and gender \cite{Argamon:2003}, 
...
In this paper, we show that a linear classifier informed by stylistic features can distinguish between the different endings to a large degree, even without looking at the story context (64.5--75.6\% binary classification results).
...
story cloze task \nascomment{cite the challenge?  a reader may
  not realize that there is a shared task on this}\roy{Maybe ``challenge'' was not the right term here. We basically mean the story cloze task. The shared task was the original task w/o any modification, so I am not referring to it specifically here}. 
 Adapting our classifier to the task, we obtain 72.4\% accuracy, a
 12.5\% increase over the previously reported state of the art  \cite{Salle:2016}.
...
be used to distinguish between the different types of story endings.
...
either as right vs.~wrong or as original vs.~new.
...
\isectionb{Results}
\tabref{results} shows our results.  In both experiments,
...
were given to perform.  We further applied the model to a third task,
\emph{original} vs.~\emph{wrong (new)} endings, and even stronger
...
\end{tabular}
\end{center}
\caption{\label{results} Results of  experiments 1 ({\it right
    vs.~wrong}) and 2 ({\it original vs.~right (new)}). 
The bottom row shows an additional experiment which classifies {\it
...
set. Our classifier obtains 72.4\% accuracy, 12.5\% (absolute) higher than the
...
\end{tabular}
\end{center}
\caption{\label{cloze_results}\resolved{\yc{we should change accuracy to be in 0-100\% scale to be consistent with Table 2}}
...
model.\nascomment{added this to replace the hedging that was here before}\roy{We actually also use the RNNLM features in log space. Add that in the next paragraph.}}
...
absolute, reaching 75.2\% (15.3\% better than the published state-of-the-art\com{ and 4.1\% better than the best unpublished} result).\resolved{\roy{As now the codalab shared task table contains our numbers, I removed the references to these works, and left only the footnote earlier which says that there are higher, unpublished numbers.}}
...
classifier in both Experiments 1 and 2, an  approach widely used  as a
...
This suggests that authors are more likely to show or evoke enthusiasm when writing their own text compared to ending an existing task.

Finally, when comparing the two groups of salient features from both experiments, we find an interesting trend.
Several features, such as ``\textsc{start} NNP'' and  ``NN .'', which indicate {\it wrong} sentences in Experiment 1, are used to predict {\it new} (i.e., {\it correct}) endings in Experiment 2. 
...
The top 5 most heavily weighted features for predicting {\it original} vs.~{\it new} ({\it right}) endings.
}
\end{table}
...
One way to avoid the inherent biases people have when writing is to 
have them rate sentences from naturally occurring text by parameters
...
Some works even showed that gender affects a person's  writing style when lying \cite{Perez:2014a,Perez:2014b}.
