...
In addition, our style-based classifier's performance on the
  story cloze challenge is comparable \nascomment{why isn't this stronger?} to the state of the art.
...
\nascomment{should this be updated given the shared task, and Gimpel's
  results?} Interestingly, until very recently, one year after the task was first introduced, the published benchmark on this task was still below 60\%.
...
classification accuracy---\nascomment{is that actually higher
  (statistically speaking) than random?  maybe say ``close to a random
  baseline'' and leave it at that?} higher than a random baseline, but substantially lower than our story cloze task results. 
...
\nascomment{anyone else?  who read drafts?} This research was supported in part by DARPA under the Communicating with Computers program.
