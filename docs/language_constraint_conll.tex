...
\com{        \addplot[style={bblue,fill=bblue,mark=none}]
...
%        \legend{NN, VBD, PRP, DT, NNP}
	\legend{{\it original}, {\it right}, {\it wrong}}
    \end{axis}
\end{tikzpicture}
%\caption{
...
We train five classifiers with each of the {\it original} training sets, and report the average classification result.
...
\caption{
The top 5 most heavily weighted features for predicting {\it right} vs.~{\it wrong} endings (\ref{exp1_features}) and  {\it original} vs.~new ({\it right}) endings (\ref{exp2_features}). 
...
Another way to avoid such effects is to use naturally occurring text, as used in recent machine reading tasks (see \secref{Related}). 
...
Finally, a different approach is to have people rate sentences from naturally occurring text by parameters
