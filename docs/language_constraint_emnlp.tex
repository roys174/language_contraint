...
As a case study, we present experiments based on the recently introduced commonsense challenge---the \shortv{ROC }story cloze task \cite{Mostafazadeh:2016}. 
...
Framed as a story cloze task, the goal of this dataset is to serve as a commonsense challenge for NLP \shortv{and AI }research. 
...
Interestingly, the story cloze task annotation process introduced three variants of the same writing task: writing an ending to a short story (see \secref{ROC_Story}).
In this paper, we show that a linear classifier informed by stylistic features can distinguish among the different variants to a large degree, even without looking at the rest of the story\shortv{ (64.5--68.5\% binary classification results)}.
...
{\bf Story Prefix} & {\bf Ending} \\ \hline
\multirow{4}{*}{\parbox{4.6cm}{Joe went to college for art. He graduated with a degree in painting. He couldn't find a job. He then responded to an ad in the paper.}} & 	{\color{blue}{???}} \\ \cline{2-2}
& 	{\color{forestgreen}{Then he got hired.}} \\ \cline{2-2}
& {\color{red}{Joe hated pizza.}} \\ %\cline{2-2}
&  \\ \hline
\end{tabular}
\caption{\label{ROC-example}
Examples of stories from the story cloze task\shortv{ \cite{Mostafazadeh:2016}}. The table shows a story prefix with three contrastive endings:
 %of the same story: 
...
The task is simple}:  given a pair of stories that differ only in their endings, the system decides which \shortv{ending }is {\it right} and which is {\it wrong}. 
...
as }a commonsense\shortv{ story} understanding task\com{, 
...
While the former are part of a \shortv{single }coherent story written by the same author, the latter were generated by asking an author to read four sentences, 
and then \shortv{asking her to }generate a fifth {\it right} ending. 
...
In order to answer these questions, we use simple methods that do not capture content, and have been shown to be\shortv{ very} effective for recognizing style\shortv{ (see \secref{Related})}.
...
We experiment with an LSTM\shortv{-based} \cite{hochreiter1997long} recurrent neural network language model (LM; \citealp{mikolov2010recurrent}). % on the ROC story corpus training data,
...
model}LM.\resolved{\nascomment{reworded.  more important that our features help
  the language model, than the other way around!}}\footnote{The LM training details are in the supplementary file.}\supl{
...
$\eta=.001$, while \shortv{train to }minimizing cross-entropy.} % minibatches of $50$ stories.
% h=512, dropout rate of 60%, V=21582, 10% validation set
To apply \shortv{the RNNLM}it to the classification problem, we select
...
We combine our linear model with the \shortv{RNN}LM by adding three features to
...
absolute. Our final result---75.2\%\com{ (15.3\% better than the published state-of-the-art\com{ and 4.1\% better than the best unpublished} result)}---is a new-state-of-the-art\shortv{  for the task}.\resolved{\roy{As now the codalab shared task table contains our numbers, I removed the references to these works, and left only the footnote earlier which says that there are higher, unpublished numbers.}}
...
\roy{The COPA dataset \cite{Roemmele:2011} was also designed to test commonsense knowledge, by presenting systems with premises like {\it I put my plate in the sink.} and then letting it decide between two alternatives, e.g.: (a) {\it I finished eating.} and (b) {\it I skipped dinner.}}
Importantly, in this task,  annotators were asked to be as brief as possible and avoid proper names, as well as  slang. 
...
A different approach is to use naturally occurring text, as used in recent machine reading tasks \cite{hermann2015teaching}\shortv{ (see \secref{Related})}. 
...
In both cases, a simple \shortv{linear }model reveals measurable differences in writing styles, %that different styles are imposed.
