...
\begin{tabular}{|p{4.5cm}|p{2.4cm}|} \hline
{\bf Story Prefix} & {\bf Ending} \\ \hline
My mother loves clocks that chime.	Her house is full of them.	She sets them each a little different so she can hear them chime.	It sounds like a bell tower during a wedding in her house all day.	& {\color{blue}{When I visit I stop them or I'd never be able to sleep at night.}} \\ \hline
\multirow{2}{4.5cm}{Joe went to college for art. He graduated with a degree in painting. He couldn't find a job. He then responded to an ad in the paper.} & 	{\color{forestgreen}{Then he got hired.}} \\ \cline{2-2}
...
the \shortv{recently introduced ROC }story cloze task \cite{Mostafazadeh:2016}. 
\roy{In this task, whose goal is to serve as a commonsense challenge for NLP \shortv{and AI }research, a system is presented with two short stories that differ only in their last sentence: one ending makes the entire story {\it coherent} while the other makes it {\it incoherent}. 
...
\shortv{\tabref{ROC-example} shows an example of an {original} story, a {coherent} story, and an {incoherent} story.}
...
In this paper, we show that a linear classifier informed by stylistic features can distinguish among the different variants to a large degree, even without looking at the story context\shortv{ (64.5--68.5\% binary classification results)}.
...
We experiment with an LSTM\shortv{-based} \cite{hochreiter1997long} recurrent neural network language model (RNNLM; \citealp{mikolov2010recurrent}). % on the ROC story corpus training data,
...
as \emph{right} the ending with the higher value of\footnote{The intuition is that a \emph{right} ending should be unsurprising (to
the model)
...
One way to avoid such problems is by instructing authors to write shorter text spans, such as the ones used in the Winograd schema \cite{Levesque:2011}.
