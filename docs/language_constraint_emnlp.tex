...
different tasks likely engage different cognitive processes \cite{Campbell:2003,Banerjee:2014}.\roy{\footnote{For the purpose of this paper, \emph{style} is defined as content-agnostic writing characteristics, e.g., sentence length.}}
...
Interestingly, the story cloze task annotation process introduced three different variants of the writing task of writing an ending to a short story (\secref{ROC_Story}).
...
In order to answer these questions, we use simple methods that do not capture content, and have been shown to be effective for recognizing style\shortv{ (see \secref{Related})}.
...
along with their feature weight (Wt.)~and their frequency in our story cloze training set (Freq.).
}
\end{table}
...
While this is partially explained by the smaller data size of the COPA dataset (1,000 samples compared to 3,742 in the story cloze task), our findings also indicate that simple instructions  can help alleviate the effects of writing style found in this paper.
